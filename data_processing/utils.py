...
def generate_interior_camera_positions(num_views, radius):
   positions = []
   focal_points = []
   num_angles = 36
   num_height = int(num_views / num_angles)
   for i in range(num_height):
       phi = np.arccos((1 - 1 * i / num_height) * 0.3)
       for j in range(num_angles):
           ...
           z = radius * np.cos(phi)
           positions.append([x, y, z])
           focal_points.append([1.1*x, 1.1*y, 1.1*z])
   return positions, focal_points


def render_interior_images(pv_mesh, texture, camera_positions, intrinsics, output_dir, focal_points):
   ...
   for i, position in enumerate(camera_positions):
       try:
           focal_point = focal_points[i]  # Dynamically set the focal point
